...
async fn flash_payload_retry_and_models() {
    ...
    let (url,task)=serve(Router::new().route("/services/aigc/multimodal-generation/generation",post(move |req:Request|{let count=counter.clone();let captured=captured.clone();async move {
        assert_eq!(req.headers()["Authorization"],"Bearer upstream-key");let body=to_bytes(req.into_body(),16<<20).await.unwrap();captured.lock().unwrap().push(serde_json::from_slice::<Value>(&body).unwrap());
...
async fn oss_policy_and_resolution_header() {
    ...
    let (url,task)=serve(Router::new().route("/uploads",get(move ||{let host=upload.clone();async move {Json(json!({"data":{"upload_host":host,"upload_dir":"prefix","oss_access_key_id":"id","signature":"signature","policy":"policy"}}))}}))
        .route("/services/audio/asr/transcription",post(|req:Request|async move {assert_eq!(req.headers()["X-DashScope-OssResourceResolve"],"enable");Json(json!({"task_id":"oss"}))}))
        .route("/tasks/oss",get(move ||{let url=result.clone();async move {Json(json!({"output":{"task_status":"SUCCEEDED","results":[{"transcription_url":url}]}}))}}))).await;
    let mut cfg = config();
    cfg.dashscope_base_url = url;
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("a.ogg");
    std::fs::write(&path, b"audio").unwrap();
    assert_eq!(
        DashScope::new(Arc::new(cfg))
            .unwrap()
            .transcribe(&path, "paraformer-v2", &Options::default())
            .await
            .unwrap(),
        "OSS"
    );
